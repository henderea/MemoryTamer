...
class Persist
  class << self
    ...
    def calculated_property(*names, &getter)
      names.each { |name|
        ...
        define_method("#{name.to_s}?".to_weak) { Persist.value_to_bool(getter.call(self)) }
  ...
  validate_map(:mem) { |_, _, nv| Persist.constrain_value_range((0..MemInfo.getTotalMemory), nv, 1024) }
  validate_map(:trim_mem) { |_, _, nv| Persist.constrain_value_range((0..MemInfo.getTotalMemory), nv, 0) }
  validate_map(:auto_threshold) { |_, ov, nv| Persist.constrain_value_list(%w(low high), ov, nv, 'low') }
  validate_map(:pressure) { |_, ov, nv| Persist.constrain_value_list(%w(warn critical), ov, nv, 'warn') }
  validate_map(:growl) { |_, _, nv| Persist.constrain_value_boolean(nv, false) }
  validate_map(:method_pressure) { |_, _, nv| Persist.constrain_value_boolean(nv, true) }
  validate_map(:freeing_method) { |_, ov, nv| Persist.constrain_value_list(['memory pressure', 'plain allocation'], ov, nv, 'memory pressure') }
  validate_map(:update_while) { |_, _, nv| Persist.constrain_value_boolean(nv, false, Version.last >= '1.0b6') }
  validate_map(:grayscale_icon) { |_, _, nv| Persist.constrain_value_boolean(nv, false) }
  validate_map(:display_what) { |_, ov, nv| Persist.constrain_value_list(['Show Icon + Free Memory', 'Show Icon', 'Show Free Memory'], ov, nv, (Persist.store['show_mem'].nil? || Persist.store['show_mem?']) ? 'Show Icon + Free Memory' : 'Show Icon') }
  validate_map(:mem_places) { |_, _, nv| Persist.constrain_value_range((0..3), nv, 2) }
  validate_map(:refresh_rate) { |_, _, nv| Persist.constrain_value_range((1..5), nv, 2) }
  validate_map(:sticky) { |_, _, nv| Persist.constrain_value_boolean(nv, false) }
  validate_map(:auto_escalate) { |_, _, nv| Persist.constrain_value_boolean(nv, false) }
  validate_map(:notifications) { |_, ov, nv| Persist.constrain_value_list(['None', 'Growl', 'Notification Center'], ov, nv, 'Notification Center') }
  validate_map(:free_start) { |_, _, nv| Persist.constrain_value_boolean(nv, true) }
  validate_map(:free_end) { |_, _, nv| Persist.constrain_value_boolean(nv, true) }
  validate_map(:trim_start) { |_, _, nv| Persist.constrain_value_boolean(nv, true) }
  validate_map(:trim_end) { |_, _, nv| Persist.constrain_value_boolean(nv, true) }
  ...
  def load_prefs
    self.no_refresh {
      Version.last = self.last_version
      self.last_version = Version.current.to_s
